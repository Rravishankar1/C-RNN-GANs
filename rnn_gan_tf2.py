...
class RNNGAN(object):
  ...
  def __init__(self, is_training, num_song_features=None, num_meta_features=None):
    ...
    with tf.variable_scope('G') as scope:
      ...
      if False: #FLAGS.generate_meta:
        print("Running generate meta")
  ...
  def discriminator(self, inputs, is_training, msg=''):
    ...
    if not FLAGS.unidirectional_d:
      ...
      outputs, state_fw, state_bw = tf.nn.static_bidirectional_rnn(cell_fw, cell_bw, inputs, initial_state_fw=self._initial_state_fw, initial_state_bw=self._initial_state_bw)
...
def run_epoch(session, model, loader, datasetlabel, eval_op_g, eval_op_d, iterations, pretraining=False, verbose=False, run_metadata=None, pretraining_d=False):
  ...
  batch_song = loader.get_batch(model.batch_size, part=datasetlabel)
...
def main(_):
  ...
  loader = loader_utils.Loader(FLAGS.datadir, FLAGS.select_validation_percentage, FLAGS.select_test_percentage)
  if FLAGS.synthetic_chords:
    # This is just a print out, to check the generated data.
    batch = loader.get_batch(batchsize=1)
    loader.get_midi_pattern([batch[1][0][i] for i in xrange(batch[1].shape[1])])

  num_song_features = loader.get_num_features()
  ...
  if global_step < FLAGS.pretraining_epochs:
    ...
    FLAGS.songlength = loader.days_per_series
    ...
    if not FLAGS.sample:
      ...
      for i in range(global_step, FLAGS.max_epoch):
        ...
        if global_step < FLAGS.pretraining_epochs:
          ...
          new_songlength = loader.days_per_series #int(min((i+1)*4,songlength_ceiling))
        else:
          new_songlength = loader.days_per_series #songlength_ceiling
        ...
        if i<FLAGS.pretraining_epochs:
          ...
          if FLAGS.pretraining_d:
            opt_d = m.opt_d
          train_g_loss,train_d_loss = run_epoch(session, m, loader, 'train', m.opt_pretraining, opt_d, 1, pretraining = True, verbose=True, run_metadata=run_metadata, pretraining_d=FLAGS.pretraining_d)
          if FLAGS.pretraining_d:
            try:
          ...
          train_g_loss,train_d_loss = run_epoch(session, m, loader, 'train', m.opt_d, m.opt_g, 1, verbose=True, run_metadata=run_metadata)
          try:
        ...
        valid_g_loss,valid_d_loss = run_epoch(session, m, loader, 'validation', tf.no_op(), tf.no_op(), 1)
        try:
      ...
      test_g_loss,test_d_loss = run_epoch(session, m, loader, 'test', tf.no_op(), tf.no_op(), 10)
